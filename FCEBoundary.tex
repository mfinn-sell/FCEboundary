...
A natural corollary of this concerns coarse embeddings, which was a question initially raised by Roe \cite{}:

\begin{corollary}\label{thm:IT3}
If $X$ coarsely embeds into Hilbert space then $K_{*}(I_{G}) \cong K_{*}(\mathcal{K})$. \qed
\end{corollary}

In particular Proposition \ref{thm:MT2} for the spaces of graphs that coarsely embed into Hilbert space but do not have property A \cite{,} implies that these spaces do not have any non-compact ghost projections. 
...
It is well known that the boundary groupoid associated to a box space $\square\Gamma$ decomposes as $\partial\beta \square \Gamma \rtimes \Gamma$, for a proof see \cite{mypub1}. Lemma \ref{lem:cor2}, plus a result of \cite{BG-action-2012} allows us to prove:
